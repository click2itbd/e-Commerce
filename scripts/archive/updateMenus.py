...
with open('src/pages/admin/tabs/menus/Menus.tsx', 'r', encoding='utf-8') as f:
    content = f.read()
...
content = content.replace(
    "return () => { unsubMenus(); unsubBrands(); unsubModels(); unsubVariants(); };",
    "return () => { unsubMenus(); unsubBrands(); unsubModels(); unsubVariants(); unsubProducts(); };"
)

content = content.replace(
    "const [activeForm, setActiveForm] = useState<'category' | 'subcategory' | 'brand' | 'model' | 'variant' | null>(null);",
    "const [activeForm, setActiveForm] = useState<'category' | 'subcategory' | 'brand' | 'model' | 'variant' | string | null>(null);"
)

content = content.replace(
    "const [variantForm, setVariantForm] = useState<Partial<Variant>>({ modelId: '', attributeName: '', attributeValue: '', sku: '', price: 0, stock: 0, barcode: '', status: 'Active' });",
...
with open('src/pages/admin/tabs/menus/Menus.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
