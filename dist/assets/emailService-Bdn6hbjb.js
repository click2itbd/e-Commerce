import{query as m,collection as c,where as g,getDocs as f,addDoc as d}from"./firebase-core-Cm5yDKb8.js";import{d as l,e as u}from"./index-B0OuF8bX.js";const w="https://click2itbd.com";async function h(s,e={}){const n=w.replace(/\/+$/,"");let o=s.startsWith("/")?s:`/${s}`;n.endsWith("/api")&&o.startsWith("/api/")&&(o=o.slice(4));const t=`${n}${o}`,r={"Content-Type":"application/json",...e.headers||{}};if(!r.Authorization&&typeof window<"u"&&u.currentUser)try{const i=await u.currentUser.getIdToken();i&&(r.Authorization=`Bearer ${i}`)}catch{}const a=await fetch(t,{...e,headers:r});if(!a.ok){const i=await a.json().catch(()=>({error:"Network error"}));throw new Error(i.error||`HTTP ${a.status}`)}return a.json()}const A=async(s,e,n)=>{const o=`Your Service is Active! - ${n.domain||"Hosting"}`,t=`
Hello,
