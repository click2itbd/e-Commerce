import{query as m,collection as c,where as g,getDocs as f,addDoc as d}from"./firebase-core-CqvQgLvb.js";import{d as l,e as u}from"./index-BkTWs-ly.js";const w="https://click2itbd.com";async function h(s,t={}){const n=w.replace(/\/+$/,"");let o=s.startsWith("/")?s:`/${s}`;n.endsWith("/api")&&o.startsWith("/api/")&&(o=o.slice(4));const r=`${n}${o}`,e={"Content-Type":"application/json",...t.headers||{}};if(!e.Authorization&&typeof window<"u"&&u.currentUser)try{const i=await u.currentUser.getIdToken();i&&(e.Authorization=`Bearer ${i}`)}catch{}const a=await fetch(r,{...t,headers:e});if(!a.ok){const i=await a.json().catch(()=>({error:"Network error"}));throw new Error(i.error||`HTTP ${a.status}`)}return a.json()}const A=async(s,t,n)=>{const o=`Your Service is Active! - ${n.domain||"Hosting"}`,r=`
Hello,
