...
const HostingPlansTab: React.FC = () => {
  ...
  const handleSavePackage = async (e: React.FormEvent) => {
    ...
    try {
      if (editingPackage) {
        await updateDoc(doc(db, 'hostingPlans', editingPackage.id), { ...packageForm, updatedAt: new Date().toISOString() });
        toast.success('Package updated');
      } else {
        await addDoc(collection(db, 'hostingPlans'), { ...packageForm, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() });
