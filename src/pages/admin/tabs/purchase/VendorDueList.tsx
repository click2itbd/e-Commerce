...
import { Vendor, Transaction, PaymentAccount } from '../../../../types';

export default function VendorDueList() {
  ...
  const [selectedVendor, setSelectedVendor] = useState<{ id: string, name: string, due: number } | null>(null);
  ...
  const fetchData = async () => {
    try {
      ...
      setVendors(custSnap.docs.map(d => ({ id: d.id, ...d.data() } as Vendor)));
  ...
  // Calculate dues
  // A positive balance indicates the vendor owes us money (Due)
  const calculateDues = () => {
    const balances: Record<string, number> = {};
    
    transactions.forEach(t => {
      if (t.entityType === 'vendor' && t.entityId) {
        if (!balances[t.entityId]) balances[t.entityId] = 0;
        
        if (t.type === 'purchase') {
          balances[t.entityId] += Number(t.amount);
        } else if (t.type === 'payment_made' || t.type === 'purchase_return') {
          balances[t.entityId] -= Number(t.amount);
        }
      }
    });

    const dueList = vendors
      .map(c => ({
        ...c,
        due: balances[c.id] || 0
      }))
  ...
  const dueVendors = calculateDues().filter(c => 
    c.name.toLowerCase().includes(searchTerm.toLowerCase()) || 
    (c.phone && c.phone.includes(searchTerm))
  );

  const totalOverallDue = dueVendors.reduce((acc, curr) => acc + curr.due, 0);
  ...
  const handleMakePayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const txData = {
        ...
        description: paymentNotes || 'Received vendor due payment',
        ...
        previousDue: selectedVendor.due,
        currentBalance: selectedVendor.due - Number(paymentAmount)
      };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="p-6 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
              <CreditCard className="text-blue-600" /> Vendor Due List
            </h2>
            <p className="text-sm text-gray-500 mt-1">Manage and collect outstanding payments from vendors</p>
          </div>
                ...
                dueVendors.map((vendor) => (
                  <tr key={vendor.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right font-black text-red-600">
                      {formatCurrency(vendor.due, settings)}
                    </td>
                    <td className="px-6 py-4 text-center">
                      <div className="flex justify-center gap-2">
                        <button
                          onClick={() => setHistoryVendor({ id: vendor.id, name: vendor.name })}
                        ...
                        <button
                          onClick={() => {
                            setSelectedVendor({ id: vendor.id, name: vendor.name, due: vendor.due });
                            setPaymentAmount(vendor.due);
                          }}
      ...
      {selectedVendor && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleMakePayment} className="p-6 overflow-y-auto space-y-4">
              <div className="bg-blue-50 text-blue-800 p-4 rounded-lg border border-blue-100 mb-2">
                <div className="text-xs font-bold uppercase text-blue-600 mb-1">Vendor</div>
                <div className="font-black text-lg">{selectedVendor.name}</div>
                <div className="text-sm mt-1 flex justify-between">
                  <span>Current Due:</span>
                  <span className="font-bold text-red-600">{formatCurrency(selectedVendor.due, settings)}</span>
                </div>
              </div>
      ...
      {historyVendor && (() => {
        ...
        const vendorTx = [...transactions]
          .filter(t => t.entityId === historyVendor.id)
          .reverse()
          .map(t => {
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
              <div className="flex justify-between items-start p-6 border-b border-gray-100 bg-gray-50">
                <div>
                  ...
                  <p className="text-sm text-gray-500 font-medium mt-1">{historyVendor.name}</p>
                </div>
                ...
                <button 
                  onClick={() => {
                    ...
                    const html = `
                      <html>
                        <head>
                          <title>Ledger - ${historyVendor.name}</title>
                          <style>
                        ...
                        <body>
                          <h1>Ledger History: ${historyVendor.name}</h1>
                          <div class="summary">
