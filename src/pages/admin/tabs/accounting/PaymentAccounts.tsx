...
const PaymentAccountsTab: React.FC = () => {
  ...
  const fetchPaymentAccounts = async () => {
    try {
      const q = query(collection(db, 'payment_accounts'), orderBy('name'));
      const snap = await getDocs(q);
      setPaymentAccounts(snap.docs.map(d => ({ id: d.id, ...d.data() } as PaymentAccount)));
    } catch (err) {
      console.error(err);
  ...
  const handleDeletePaymentAccount = async (id: string) => {
    ...
    try {
      ...
      setPaymentAccounts(paymentAccounts.filter(p => p.id !== id));
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
      ...
      {isAddingPaymentAccount ? (
        <div className="p-6">
          <div>
            <h3 className="text-xl font-bold mb-6 text-gray-800">Add Payment Method</h3>
            <form onSubmit={handleAddPaymentAccount} className="space-y-6">
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b border-gray-100 text-[10px] font-bold text-gray-500 uppercase tracking-wider">
              <tr>
                ...
                <th className="px-6 py-3 text-right cursor-pointer select-none hover:bg-gray-100 transition-colors" onClick={() => setPaymentAccountSort({ key: 'balance', direction: paymentAccountSort.key === 'balance' && paymentAccountSort.direction === 'asc' ? 'desc' : 'asc' })}>
              ...
              {[...paymentAccounts].sort((a,b) => {
                ...
                <tr key={account.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                    <button onClick={() => handleDeletePaymentAccount(account.id)} className="text-red-500 hover:text-red-700 p-1">
                      <Trash2 size={16} />
                    </button>
                  </td>
                </tr>
