const fs = require('fs');
let code = fs.readFileSync('src/pages/admin/tabs/hosting/HostingPlans.tsx', 'utf8');
...
const seedCode = `
  const handleSeed = async () => {
    ...
    try {
      ...
      for(let p of plans) await setDoc(doc(db, 'hosting_plans', p.id), p);
...
code = code.replace('// --- FEATURE HANDLERS ---', seedCode + '\n  // --- FEATURE HANDLERS ---');
code = code.replace(
...
fs.writeFileSync('src/pages/admin/tabs/hosting/HostingPlans.tsx', code);
