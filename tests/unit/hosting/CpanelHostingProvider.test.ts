...
describe('CpanelHostingProvider', () => {
  ...
  it('testConnection calls correct WHM endpoint with Authorization header', async () => {
    const { CpanelHostingProvider } = await import(
      '@functions/providers/hosting/CpanelHostingProvider'
    );
    ...
    expect(url).toBe('https://server2025.click2it.bd:2087/listaccts?api.version=1');
    expect(options.method).toBe('GET');
    expect(options.headers.Authorization).toBe('whm TEST_WHM_TOKEN');
  ...
  it('never logs or returns the actual token', async () => {
    ...
    expect(result.message).not.toContain('whm ');
  });
});
