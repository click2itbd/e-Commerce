...
function findPropSource(propName) {
  ...
  const common = ['products', 'orders', 'customers', 'vendors', 'transactions', 'settings', 'loading', 'hasPermission', 'isAdmin', 'formatCurrency', 'cn', 'toast', 'fetchData', 'setActiveTab', 'setConfirmModal', 'fileInputRef', 'menus', 'discountCodes', 'soldSerials', 'serviceRecords', 'hostingPlans', 'hostingServices', 'employees', 'employeeLeaves', 'employeeSalaries', 'paymentAccounts', 'transactionCategories', 'campaigns', 'serialSelectionModal', 'conveyances', 'isAddingConveyance', 'newConveyance', 'isAddingManualTransaction', 'manualTransactionType', 'newManualTransaction', 'isAddingTransactionCategory', 'transactionCategories', 'newTransactionCategory', 'isAddingService', 'editingService', 'serviceFormData', 'ledgerView', 'ledgerSearchQuery', 'isAddingMenu', 'subCategoryFormData', 'editingMenu', 'setEditingMenu', 'setSubCategoryFormData', 'isAddingSubCategory', 'setIsAddingSubCategory', 'selectedProductIds', 'isBulkEditing', 'bulkEditData', 'isUploading', 'dragOver', 'isAddingProduct', 'editingProduct', 'formData', 'inventoryCategoryFilter', 'selectedOrderIds', 'isRecordingPayment', 'paymentFormData', 'saleData', 'saleDiscountCodeInput', 'purchaseData', 'isCreatingSale', 'isCreatingPurchase', 'showPCBuilderModal', 'purchaseStartDate', 'purchaseEndDate', 'purchaseSearchQuery', 'orderStatusFilter', 'orderSort', 'orderSearchQuery', 'orderStartDate', 'orderEndDate', 'isAddingCampaign', 'editingCampaign', 'campaignFormData', 'isAddingDiscountCode', 'editingDiscountCode', 'discountCodeFormData', 'isAddingHostingPlan', 'editingHostingPlan', 'hostingPlanFormData', 'isAddingHostingService', 'editingHostingService', 'hostingServiceFormData', 'isAddingUser', 'userFormData', 'editingUserPermissions', 'showPermissionsModal', 'paymentAccountSort', 'paymentAccountFormData', 'settingsTab', 'settingsFormData', 'taxCalcAmount', 'serviceRecords', 'isAddingService', 'editingService', 'selectedCustomerId', 'isAddingEmployee', 'editingEmployee', 'employeeFormData', 'isAddingLeave', 'editingLeave', 'leaveFormData', 'isAddingSalary', 'editingSalary', 'salaryFormData'];
...
for (const [tabName, componentInfo] of Object.entries(tabComponents)) {
  ...
  // Find the start of this tab's content - handle both with and without && conditions
  let startMarker = `) : activeTab === '${tabName}' ? (`;
  let startIdx = content.indexOf(startMarker);
  
  if (startIdx === -1) {
    // Try with hasPermission/isAdmin conditions
    startMarker = `) : activeTab === '${tabName}' && `;
    startIdx = content.indexOf(startMarker);
    if (startIdx !== -1) {
      // Find the `? (` after the condition
      const qIdx = content.indexOf('? (', startIdx);
      if (qIdx !== -1) {
        startMarker = content.slice(startIdx, qIdx + 3);
        startIdx = startIdx;
      } else {
        console.log(`Skipping ${tabName}: no ? ( found after condition`);
        continue;
      }
    } else {
  ...
  if (nextMatch) {
    ...
    // No more tabs - this is the last one
